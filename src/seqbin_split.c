...
static int uniq_flag = 1;
static int cons_flag = 0;
static int min_length = 20;
static int N_outread = 40000;
...
int main(int argc, char **argv)
{
    ...
    void Align_Process(char **argv,int args,int argc,int nRead);
...
//    system("ps aux | grep eulerSNP; date");
    ...
    Align_Process(argv,args,argc,num_reads);
    return EXIT_SUCCESS;
...
/* ====================================================  */
{
     int i;
//     char **rdname;
     char *ptr,line[2000000],Nameout[100],RC;
     FILE *namef,*namef2;
     long num_align=0;
     int ac,name_flag=0;
...
/*   read the cigar line file   */
   ...
   {
     if((namef2 = fopen(argv[ac],"r")) == NULL)
     {
     ...
     while(!feof(namef2))
     {
       int nPair=0,add_flag=0,len,hit1,hit2,m_score;
      
       fgets(line,2000000,namef2);
       if(feof(namef2)) break;
       if(((num_out%N_outread)==0)&&((num_align%4)==0))
       {
...
//       if((num_align+1)%4==0)
//         line[0] = '!';
//       printf("%d %d %d %s %s",num_out,num_align,(num_align+1)%4,Nameout,line);
       fprintf(namef,"%s",line);
       if(((num_out%N_outread)==0)&&(out_flag==1))
       {
         if(((num_align+1)%4)==0)
         {
//       printf("www: %d %d %d %s %s",num_out,num_align,num_out%N_outread,Nameout,line);
           fclose(namef);
         }
       }
       if((num_align%4)==0)
...
//     fclose(namef);
     ...
     printf("number of reads processed: %ld\n",num_align);
