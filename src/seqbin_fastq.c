...
{
    int i,nSeq,args,n_files,n_r1,n_r2,nseq;
    char *st,*ed;
...
//      printf("System command error:\n);
    ...
    for(i=0;i<n_r1;i++)
    {
       ...
       memset(syscmd,'\0',2000);
       sprintf(syscmd,"%s/scaff_split -split 20000000 split-reads_1 %s > try.out",bindir,R1_Name1[i]);
       RunSystemCommand(syscmd);

       memset(syscmd,'\0',2000);
       sprintf(syscmd,"%s/scaff_split -split 20000000 split-reads_2 %s > try.out",bindir,R2_Name1[i]);
       RunSystemCommand(syscmd);
