...
static int *hit_index,*hit_salign,*hit_mtags,*hit_mscore;
...
/*
    ...
    {
      printf("fmate: calloc - hit_mcsore\n");
      exit(1);
    }
    ...
    if((hit_salign = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - hit_salign\n");
      exit(1);
    }
    if((hit_mtags = (int *)calloc(nseq,sizeof(int))) == NULL)
    {
      printf("fmate: calloc - hit_salign\n");
      exit(1);
    }
...
/*  read the alignment files         */
    i=0;
    while(fscanf(namef,"%d %d %s %s %d",&hit_mscore[i],&hit_salign[i],R_Name[i],S_Name[i],&hit_mtags[i])!=EOF)
    {
        int idt;
...
/* =============================== */
{
     ...
     for(i=0;i<(nSeq-1);i++)
     {
	  ...
	  for(n=i;n<j;n++)
	  {
             if((hit_index[n] >= 0)&&((hit_mscore[i] > 0)||(hit_mscore[i+1] > 0)))
             {
               if(strcmp(T_Name[n],"MAT") == 0)
//               if(hit_index[n] <= 31)
               {
                 n_Hap1 = n_Hap1+hit_salign[n]; 
                 m_Hap1 = m_Hap1+hit_mscore[n]; 
               }
               else
               {
                 n_Hap2 = n_Hap2+hit_salign[n]; 
                 m_Hap2 = m_Hap2+hit_mscore[n]; 
               }
             }
          }
//          printf("www: %s %d %d %d %d\n",R_Name[i],n_Hap1,n_Hap2,m_Hap1,m_Hap2);
          if((m_Hap2 == 0)&&(m_Hap1 == 0))
          {
            fprintf(namef,"%s\n",R_Name[i]);
            fprintf(namef2,"%s\n",R_Name[i]);
          }
          else if((n_Hap2 == 100)&&(n_Hap1 == 100))
          {
            if(m_Hap1>m_Hap2)
              fprintf(namef,"%s\n",R_Name[i]);
            else if(m_Hap1<m_Hap2)
              fprintf(namef2,"%s\n",R_Name[i]);
            else
            {
              fprintf(namef,"%s\n",R_Name[i]);
        ...
        {
//          printf("www: %s %s %d %f\n",R_Name[i],T_Name[i],hit_mtags[i],0.0);
        }
	num_hits = j-i;
        i=j-1;
     }
